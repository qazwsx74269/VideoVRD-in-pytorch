...
class Model_attention6(BasicModule):
	def __init__(self,param):
		super(Model_attention6,self).__init__()
		self.param = param
	...
	def forward(self,f,keys):
		...
		Q = F.relu(self.qfc(inp_f))
		
		Q = Q.view(-1,1 , size_per_head)
		K = F.relu(self.kfc(inp_f))
		
		K = K.view(-1, size_per_head, 1)
		V = F.relu(self.vfc(inp_f))
		
		V = V.view(-1, 1,size_per_head)
