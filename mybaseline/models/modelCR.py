...
class Model_CR(BasicModule):
	def __init__(self,param):
		super(Model_CR,self).__init__()
		self.param = param
		...
		self.fc = nn.Linear(70+3000,self.predicate_num,bias=True)
	...
	def forward(self,f,keys):
		...
		f_ = np.concatenate((f[:,:70],f[:,8070:11070]),axis=1)
		inp_f = Variable(t.from_numpy(f_)).cuda()
		p = self.fc(inp_f)
