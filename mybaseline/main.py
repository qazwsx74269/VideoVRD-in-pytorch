	...
	for i in range(param.max_iter):
		try:
			...
			if i % param.display_freq == 0:
					print('{}-{}-use_m:{}-use_gt:{}-vtranse:{}-iter: {}/{} - loss: {:.4f}'.format(
							strftime('%Y-%m-%d %H:%M:%S'), param.model_name,param.use_m, param.use_gt,param.vtranse,i, param.max_iter, float(loss.data)))
...
def rank_jointly(pairs,s,p,o):
	...
	for i in range(len(pairs)):
			...
			score = s[i][top_s_ind,None,None]*p[i][None,top_p_ind,None]*o[i][None,None,top_o_ind]#compute score via all possible combinations among s,p,o which are predicted
			top_flat_ind = np.argsort(score,axis=None)[-param.pair_topk:]#top_20 score index
			top_score = score.ravel()[top_flat_ind]#top_20 score 多维降成一维
			top_s,top_p,top_o = np.unravel_index(top_flat_ind,score.shape)
			predictions.extend((
				top_score[j],
				(top_s_ind[top_s[j]],top_p_ind[top_p[j]],top_o_ind[top_o[j]]),
				tuple(pairs[i])) for j in range(top_score.size))
	return predictions

# def rank_independently(pairs,s,p,o):
# 	predictions = []
# 	for i in range(len(pairs)):
# 			top_s_ind = np.argsort(s[i])[-param.pair_topk:]
# 			top_p_ind = np.argsort(p[i])[-param.pair_topk:]
# 			top_o_ind = np.argsort(o[i])[-param.pair_topk:]
# 			predictions.extend((
# 				s[top_s_ind[j]]*p[top_p_ind[j]]*o[top_o_ind[j]],
# 				(top_s_ind[j],top_p_ind[j],top_o_ind[j]),
...
def eval(dataset,model):
	...
	while data:
		...
		o = feats[:,35:70]
		# if param.use_m or param.vtranse:
		# 	predictions = rank_independently(pairs,s,p,o)
		# else:
		# 	predictions = rank_jointly(pairs,s,p,o)
		predictions = rank_jointly(pairs,s,p,o)
